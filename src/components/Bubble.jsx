...
class BubblePointsVsValue extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (this.props.position && this.props.position.length > 0) {
            console.log('posisi', this.props.position)
            dataFilter = dataFilter.filter(data => this.props.position === data.position);
        }
        const selectCollections = dataFilter.slice(0, this.props.size || 50)
            const lower = Math.floor(Math.min(...selectCollections.map(data => data.points)) - 2);
            const upper = Math.ceil(Math.max(...selectCollections.map(data => data.cost)) + 2);
